...
## 소유 파일 (이 밖은 절대 수정 금지)
- `system/ingest/` — 카메라별 GStreamer 파이프라인, FrameQueue, SourceReconnector, analyze_fps 게이트
...
- 디코드: GStreamer(PyGObject) + NVDEC — M0에서 확정된 스택 사용. GLib MainLoop 스레드 + appsink 콜백 → `FrameItem{cam_id, ts, frame(ndarray BGR)}` 를 thread-safe 큐로.
- 동시성: GStreamer 스레드들 → `queue.Queue(maxsize=64)` oldest-drop → 분석 스레드 1개. asyncio 아님(FastAPI 통합은 메인 세션 몫).
...
## 검증 (작업마다)
- 모의 RTSP는 `rtsp-stream` 스킬(mediamtx+pm2)로 송출해 테스트. 16ch까지 스케일 확인.
