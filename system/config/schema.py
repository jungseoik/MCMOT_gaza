...
class Thresholds(BaseModel):
    ...
    idr_mean_align: bool = False  # IDR 판정에 **구역 평균 정렬도** a_e >= a_th 를 추가로 요구.
                           # False(기본) = 개별 객체 정렬도 비율 r_e 만 본다.
                           # a_e 는 코사인 스칼라 평균이라 역행자 1명이 순행자 1명을 상쇄해
                           # 노이즈에 약하고, r_e 가 이미 객체별로 a_i >= a_th 를 검사하므로
                           # 중복이다(실측: a_e 충족 1% vs r_e 충족 2%로 평균 쪽이 더 빡빡).
                           # True 로 두면 v1.13 이전 판정식으로 복귀 — 비교·회귀용.
    d_allow: float = 4.0   # EPFI 허용 이탈거리 (m) — **경로 중심선에서 한쪽 반경**.
                           # 이탈거리 d 는 중심선까지의 부호 없는 최단거리라 좌/우 구분이 없다.
                           # "허용 통로 폭"으로 읽으면 좌우 합 2×d_allow (4.0 → 폭 8m).
                           # 2.0 은 기준으로는 맞지만 EPFI 가 하한에 몰린다 — AI hub 실측
                           # 1233명에서 0점이 45.5%(중앙 이탈 1.79m). 4.0 이면 0점 18.6%로
                           # "얼마나 벗어났나"의 순위가 살아난다. 판정선이 필요하면 2.0.
