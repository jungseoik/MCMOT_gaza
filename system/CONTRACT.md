...
>   GPU 0) · 코사인 매칭(`cos_th` 기본 0.55) · TTL(기본 600s) · **속도 게이트**
>   (`max_speed_mps` 기본 3.0 — 마지막 위치→새 등장의 암시 속도가 보행 한계 초과면
>   기각: 닮은 두 사람의 핑퐁 스왑 차단, 실측 117m/23s 가짜 여정 소멸) ·
>   **최소 관측수**(`min_new_obs` 기본 3 — 유령 트랙 id 남발 방지) ·
>   프로토타입 ≤3개(시점 흡수) · **동시 활성 기각**
>   (CJ 실측: 0.45 단독=오병합으로 통과 5→3 증발 / 0.65=조각화 / 0.55+게이트=통과
>   기준값 유지·핑퐁 0·여정 31→22)
