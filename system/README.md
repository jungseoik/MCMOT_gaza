...
- 평가 세션: 운영 뷰 🔔 경보 시작(맵 클릭) → 4대 지표 카드 → 종료 시 결과 산출,
  ...
  **계약 v1.12** — CBS 병목은 **부채꼴 영역·그룹 집계**(선택 병목만 합계·평균 — 선택 패널은
  ③ 운영뷰·④ 리플레이 **공용** `CbsBnPanel`), SEI 출입구는
  **문별 유효폭(W)·통과 기준(q_design)** 을 문마다 지정(`CONTRACT.md`).
  세션·건물 훈련 **결과 모달은 해석형 리포트** — 종합평(잘된 점/개선 지점) + 지표별
  설명·자동 해석 문장 + 출구 분포 막대. 지표를 모르는 담당자도 결과를 읽을 수 있게 한다
- **글로벌 ID (계약 v1.13, 기본 off)**: ① [추론 모델] 패널 토글 — 켜면 카메라 간
  동일인을 외형 특징(트래커 EMA 임베딩 재사용)으로 연결해 같은 사람은 어느 카메라든
  같은 `gN`, 결과에 **개인 이동 기록(여정)**(시작 구역→최초 퇴장 출구·이동거리·소요·
  평균속도·커버리지) 포함. 전제=배타 헐(카메라 영역 무중첩), 동시 활성 기각으로 오병합
  방어. off 면 기존 카메라별 ID 그대로. 설정 `data/global_id.json` ·
  `GET/POST /api/infer/global_id`. DS 라이브 경로는 후속(그 경로는 on 이어도 기존 동작)
