...
8. **[08-훈련영상-동기송출-설계.md](08-훈련영상-동기송출-설계.md)**
   — 훈련 영상 N개를 **하나의 버튼으로 동시에 t=0부터** RTSP 송출(`system/vsource/`).
   현행 `pm2 + -stream_loop -1`은 영상 길이가 서로 달라(최장−최단 97.6s) 루프마다 어긋나
   시간축 동기가 원천 불가 — 실측 누적 드리프트 11,000분. **wall-clock 배리어**로 6채널
   실측 편차 **1.7ms** 확인. 매핑(②)이 라이브 스트림을 요구하므로 **대기 송출 / 동기 시작
   2단계**로 나눈다. **설계 — 미구현(2026-08-24).**
