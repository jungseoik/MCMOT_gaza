...
## 6-1. 구역을 어떻게 나눠야 하나 (2026-08-27 추가)

IDR 은 **구역마다 따로** 산출된다(§3.7 의 $t_{e,\mathrm{start}}$, §4.2 의
$\mathrm{IDR}_{e,j}$). 그래서 구역을 어떻게 나누느냐가 결과의 의미를 좌우한다.

### 왜 "구역별"이어야 하는가

§3.6 의 세 조건은 전부 **그 구역 안 사람들만** 놓고 계산한다.

$$v_e(t),\; a_e(t),\; r_e(t) \;\longleftarrow\; \mathcal{I}_e(t)\ \text{(구역 e 안 객체)}$$

경보가 울리면 가까운 곳부터 반응이 퍼진다. 구역이 나뉘어 있으면 그 **퍼지는
순서가 그대로 보인다** — 그게 지표 이름의 "확산"이다.

### 도면 전체를 한 구역으로 덮으면

세 조건이 **건물 전체 인원**에 대해 계산된다. 그러면:

1. **$r_e$ 가 전체 비율이 된다.** 경보 근처 3명이 즉시 움직여도 25명 중 3명
   = 12% 라 $r_{\mathrm{th}}=0.5$ 에 못 미친다. 절반이 움직일 때까지 기다리므로
   $t_{e,\mathrm{start}}$ 는 **가장 빠른 반응이 아니라 중간쯤**이 된다.
2. **$D$ 가 면적 평균이 된다.** §4.1 이 구역 내 모든 셀의 평균이므로,
   도면 전체면 "경보원에서 도면 각 지점까지의 평균 거리"다. 100×100 m 에서
   79.1 m — 실제 개시된 어느 구역보다도 멀다(최대 71.2 m).
3. 결과적으로 **IDR 이 부푼다.** 실측 5.25 → 11.29 (2.15배).
4. **확산 패턴이 사라진다.** 숫자 하나뿐이라 3초에 시작한 곳도 18초 걸린 곳도
   안 보인다 — 개선이 필요한 곳이 정확히 그 늦은 구역인데.

실측 (사람 25명 · 100×100 m · 인지 확산 8 m/s):
...
너무 잘게 쪼개면 사람이 $\Delta t_{\mathrm{hold}}$ 동안 머물지 않고 지나가
개시 판정이 안 선다(5×5 는 25개 중 7개, 10×10 은 100개 중 8개만 판정).

**권고 — 구역당 3~6명이 남는 크기.** 실무적으로는 방·복도·계단실 같은
**공간의 의미 단위**로 나누면 대체로 이 범위에 들어온다.
