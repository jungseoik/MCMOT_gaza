...
| 격자 셀 크기 $c$ [m] | 격자 최단거리 해상도 — `grid.cell_size_m` (기본 2.0 m). 맵 전체를 정사각형 셀로 자동 분할 (Option C 배경 격자) |
| 구역 크기 (권고) | **구역당 3~6명이 남는 크기.** 너무 크면 확산 패턴이 평균에 묻히고 $D$ 가 면적 평균이 되어 부푼다. 너무 작으면 사람이 $\Delta t_{\mathrm{hold}}$ 동안 머물지 않고 지나가 개시 판정이 안 선다(실측: 5×5 타일에서 25개 중 7개만 판정). 방·복도 같은 **공간의 의미 단위**로 나눈다. **도면 전체를 한 구역으로 덮는 것은 피한다** — §6-1 |
...
경보 시점 $t_{\mathrm{alarm}}$ 에 구역 $e$ 안에 있던 **사람들이 밟고 있는 셀**의 집합:
$\mathcal{P}_e = \{\,\text{cell}(p) \mid p \in \mathcal{I}_e(t_{\mathrm{alarm}})\,\}$

$$D(e,\, S_j) = \frac{1}{|\mathcal{P}_e|} \sum_{k \in \mathcal{P}_e} d_{\mathrm{grid}}(k,\, S_j)$$

$\mathcal{P}_e = \emptyset$ (경보 시점에 그 구역에 아무도 없음) 이면 구역 면적 평균으로 폴백:

$$D(e,\, S_j) = \frac{1}{|\mathcal{C}_e|} \sum_{k \in \mathcal{C}_e} d_{\mathrm{grid}}(k,\, S_j)$$

- $d_{\mathrm{BFS}}(k, S_j)$: 격자 최단거리 (m) — **8방향 인접**, 직교 이동 $c$,
  대각 이동 $\sqrt{2}\,c$ (다익스트라)

> **2026-08-27 개정 — 4방향 → 8방향.** 원래 4방향 인접이었는데, 격자에 장애물이
> 없으므로 그 경우 홉 수가 곧 **맨해튼 거리**가 된다. 대각 방향 구역의 거리가
> 최대 $\sqrt{2}$(+41%) 부풀고(실측: 45° 방향 42.4 m → 59.0 m), $\mathrm{IDR}=D/\text{지연}$
> 이므로 IDR 도 그만큼 부푼다.
>
> 더 결정적인 문제는 **오차가 방향에 따라 달라진다**는 점이다. 같은 30 m 라도
> 정면이면 30 m, 대각이면 42 m 로 나와 **도면을 45° 돌려 그리면 IDR 이 바뀐다.**
> 이는 §1의 목적("얼마나 멀리 떨어진 곳까지")과 모순된다 — 거리는 도면을 어느
> 방향으로 그렸는지와 무관해야 한다.
>
> 8방향(대각 $\sqrt{2}$)으로 바꿔 최대 오차가 **+39.1% → ±6.5%** 가 됐다.
> 장애물이 없는 현재는 결과가 octile 거리이고, 나중에 통행 불가 셀을 넣으면
> 그대로 우회 경로 거리가 된다. 근거: `docs/reports/2026-08-27_4대지표-거리계산-검증.md`
- $|\mathcal{C}_e| = 0$ (셀 없음): Zone centroid와의 직선거리 폴백

> **2026-08-27 개정 — 면적 평균 → 사람 위치 평균.** 구역 polygon 안 모든 셀을
> 평균하면 **사람이 어디 있든 같은 값**이 나온다. 도면 전체를 한 구역으로 잡으면
> 전원이 경보 바로 옆에 있어도 79.1 m 가 나왔다(실측). §1의 정의가 "얼마나
> **멀리 떨어진 곳까지** 피난을 개시했는지"이므로, 빈 바닥이 아니라 사람이 있는
> 곳을 재야 한다.
>
> | 사람 배치 | 면적 평균(구) | 사람 위치(신) |
> |---|---|---|
> | 경보 근처에 몰림 | 79.1 m | **16.1 m** |
...
## 6-1. 구역을 어떻게 나눠야 하나 (2026-08-27)

$D$ 를 사람 위치 기준으로 바꾼 뒤(§4.1 개정), **IDR 값 자체는 구역 나눔과
거의 무관해졌다.** 사람 25명·참값 8.0 m/s 로 측정:

| 나눔 | 구역 | 개시 | 구역당 | IDR 평균 | 참값 대비 |
|---|---|---|---|---|---|
| 1×1 | 1 | 1 | 25.0명 | 8.01 | 1.00x |
| 3×3 | 9 | 9 | 2.8명 | 7.61 | 0.95x |
| 5×5 | 25 | 25 | 1.0명 | 7.53 | 0.94x |
| 10×10 | 100 | 49 | 0.2명 | 6.07 | 0.76x |

1×1 부터 5×5 까지 편차 6%. **10×10 처럼 과하게 잘게 쪼개면 무너진다** —
구역당 0.2명이라 절반이 비어 개시 판정이 안 선다.

### 그래도 하나로 덮지 않는 이유

값은 맞게 나오지만 **숫자가 하나뿐**이라 어느 구역이 늦었는지 모른다.
5×5 로 보면 지연이 공간에 퍼져 있다 — 경보 근처 3 s, 멀어질수록 17 s.
§1이 IDR 의 목적을 "반응이 느린 구역을 드러내어 훈련 취약지점을 식별"이라고
적어놨는데, 구역이 하나면 **식별할 대상이 없다.**

**권고** — 방·복도·계단실 같은 **공간의 의미 단위**로 나눈다. 구역당 몇 명이
남는지는 값의 정확도에 크게 영향을 주지 않으므로(1~25명 사이 편차 6%),
"어느 공간의 반응을 따로 보고 싶은가"로 정하면 된다. 다만 구역이 대부분
비어버릴 만큼 잘게 쪼개는 것은 피한다.

근거: `docs/reports/2026-08-27_4대지표-거리계산-검증.md`
