...
| 코드 기준 | `system/metrics/` @ `22ff47d` (작업 트리 미반영 수정 제외 — §7.4) |
...
본 보고서는 `system/metrics/` **커밋 `22ff47d` 상태**로 측정했다. 측정 시점의 작업
트리에는 **커밋되지 않은 운동학 수정**(정지 객체의 가짜 속도 억제 — 슬라이딩 윈도
중앙값 + 수 초 지평 정지 게이트)이 있었고, 이 수정은 속도·정렬도를 전면적으로 바꾸므로
**같은 세션에서도 IDR 개시 판정이 달라진다**(실측: S02·S03 이 전 구간 미개시로 이동).
검증 결과의 재현성을 위해 해당 수정을 제외하고 측정했다.
**그 수정이 반영되면 본 보고서의 경계값은 재측정 대상**이다.
