...
그래서 **여정 재구성**(ReID 후처리)으로 조각을 사람으로 묶고, 정의식대로 다시 적분했다.
...
|---|---|---:|---:|---:|
| S04 | 양성 | 4.05 m | **13.27 m** | ×3.3 |
| S05 | 양성 | 2.67 m | **12.02 m** | ×4.5 |
| S06 | 양성 | 3.49 m | **13.83 m** | ×4.0 |
| S02 | 음성 | 4.98 m | 6.38 m | ×1.3 |
| S03 | 음성 | 5.82 m | 8.48 m | ×1.5 |
...
|---|---|---:|---:|---:|---:|
| S01 | 기준 | 9.42 | 3.07 | 2명 | 2 |
| S02 | 음성 | 6.38 | 1.75 | 1명 | 0 |
| S03 | 음성 | 8.48 | 2.20 | 2명 | 0 |
| **S04** | **양성** | **13.27** | 2.85 | 4명 | 2 |
| **S05** | **양성** | **12.02** | 2.40 | 1명 | 2 |
| **S06** | **양성** | **13.83** | 2.25 | 3명 | 1 |

*판정선 = 그 시나리오 중앙값의 2배 또는 `d_allow`(4 m) 중 큰 값*

양성 3건의 최대 이탈이 **12~14 m**, 음성 2건은 **6~8 m** 로 뚜렷하게 갈린다.
...
**같은 각본인데 결과가 뒤집힌다.** 음성 S02·S03 의 중앙 이탈이 6.5~6.8 m 로
양성 S04(1.85 m)보다 크다. 이 패키지에서는 EPFI로 경로 이탈을 판정할 수 없다.
...
사람 단위로 계산하면 이탈거리가 커져 **0 점이 대량 발생**한다 — 3층 1차 S02 는
12명 전원이 0점이다. 현재 `d_allow` 4 m 는 **트랙렛 단위 이탈 분포**(중앙 1.79 m)에
...
> ## 종합 판정: **부적합 — 구현 결함**
>
> EPFI 는 현재 **경로 이탈을 구조적으로 과소평가**한다. 카메라가 바뀔 때마다
> 경로가 재배정되어, 크게 우회할수록 더 잘 가려진다(§3).
>
...
| 우선순위 | 조치 | 근거 |
|:---:|---|---|
| 1 | **경로 배정을 사람 단위로** — 여정 재구성 뒤 첫 관측 한 번만 배정 | §3.2 — 최대 4.5배 과소평가 |
| 2 | `d_allow` 재조정 — 사람 단위 이탈 분포로 다시 정한다 | §4.4 — 0점 포화 |
| 3 | 3층 1차 패키지의 매핑·경로 배치 점검 | §4.2 — 같은 각본, 반대 결과 |
| 4 | 왕복 동선은 EPFI 로 판정하지 않는다고 명시 | §4.3 — 정의상 한계 |
