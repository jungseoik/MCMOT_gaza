...
### 6.4 리플레이 재현 — 결함 확인 후 수정 (2026-09-17)

§6.2 의 시나리오별 설정 중 **출입구 게이트 오버라이드**가 세션 녹화본 스냅샷에
저장되지 않아, ④ 리플레이 [재계산] 이 훈련 실행과 다른 통과 인원을 냈다.

| | 훈련 실행 | 리플레이(수정 전) | 정답 |
|---|---|---|---|
| 3층 1차 S06 | 5 / 5 | **6 / 5** | 5 / 5 |
| 3층 1차 S07 | 10 / 0 | **10 / 2** | 10 / 0 |

원인: 녹화기가 `rt.site()` 원본을 스냅샷으로 저장했는데, 엔진은 시나리오 오버라이드가
얹힌 `_site_plus_rehearsal()` 뷰로 돌고 있었다.

**조치 완료** — 녹화기가 엔진이 쓰는 뷰를 저장하도록 고쳤고(`_attach_recorder`),
이미 녹화된 세션은 `tools/backfill_session_overrides.py` 로 스냅샷을 보정했다
(해당 2건). 재검증 결과 **훈련 실행과 리플레이가 완전히 일치**한다.

| 패키지 | 훈련 실행 | 리플레이 재계산 |
|---|---|---|
| AI hub 1·3층 | 14/14 | **14/14** |
| AI hub 3층 1차 | 8/8 | **8/8** |

본 보고서 본문의 수치는 훈련 실행 시점 기록값이며, 이제 리플레이로도 같은 값이 나온다.
