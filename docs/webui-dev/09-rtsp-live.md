...
**라이브 잡 자동 정지 (`_stop_other_live`):** 라이브 잡은 스스로 끝나지 않아, 뷰어가 탭만
닫으면 워커가 계속 돌며 모델 lock을 독점한다. 그래서 **새 잡 시작(`/start`·`/rtsp`·
`/upload`) 시 기존 라이브 잡을 정지**시켜 lock을 푼다.
...
- **뷰어 종료 ≠ 잡 종료**: 탭만 닫으면 다음 잡 시작 전까지 백그라운드로 계속 돎(자동 정지가
  다음 시작 때 정리). 완전 정리는 MJPEG 연결 끊김 감지로 자동 stop을 붙이면 됨.
