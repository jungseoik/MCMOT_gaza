...
| 버전 | 일자 | 쪽수 | 범위 | 산출물 |
|------|------|------|------|--------|
| [v0.0.7](v0.0.7/) | 2026-09-18 | A4 61쪽 | **최신 배포본.** v0.0.6 내용 + ④ 리플레이 개편([지표]·[도면 편집] 탭, 재생 시점 4대 지표·SEI 출구별·IDR 구역별 전/후 비교·표출 밀도, **도면 편집 후 재계산**), 건물 훈련 **해석형 롤업 리포트**([종합]·[ID 재구성] — 여정 재구성·사람별 지표·EPFI 두 기준), **글로벌 ID** 스위치, 추론 모델 3종(실험 RT-DETRv4), ⑤ 리허설 4단계 절차, 기본 임계값 갱신(v_th 0.5·a_th 0.707·r_th 0.7·d_allow 4.0·min_box_h). 전체 화면 재캡처 | PDF·DOCX (HF `manual/v0.0.7/` — `fetch_assets.sh --manual`) |
| [v0.0.6](v0.0.6/) | 2026-08-28 | A4 53쪽 | v0.0.5 내용 + 운영 뷰 **[표시] 레이어 토글**(구역·경로·병목·출입구·경보원 개별 on/off — 지표 무영향)·**리허설(파일 모드)** 절 신설. 전체 화면 재캡처 | PDF·DOCX (HF `manual/v0.0.6/` — `fetch_assets.sh --manual`) |
...
| [v0.0.5](v0.0.5/) | 2026-08-21 | (미빌드) | v0.0.4 + **병목 부채꼴 도구**·**병목 그룹 집계**(선택한 병목만 합계·평균), **출입구 유효폭 수동 지정**·**문별 q_design**(설계 통과용량 자동 산출). 산출물 미생성 — 이 내용은 **v0.0.6에 편입**되어 배포됨 | 원고만 (v0.0.6으로 대체) |
| [v0.0.4](v0.0.4/) | 2026-08-20 | A4 50쪽 | v0.0.3 + **추론 모델 선택**(검출·ReID 조합 전환)·**출입구 화면 영역 집계**(문이 화각 가장자리일 때), 사이트 설정 3탭(설정·지표 설명·추론 모델) | [PDF](v0.0.4/피난훈련-정량평가-시스템-사용-가이드_v0.0.4.pdf) · [DOCX](v0.0.4/피난훈련-정량평가-시스템-사용-가이드_v0.0.4.docx) |
