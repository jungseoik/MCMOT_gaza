...
> 🏢 **다중 도면(N개 층)**: 편집기는 **맵 설정에서 현재 선택된 층**의 맵으로 저장·반영된다.
> 18F 도면을 만들려면 **먼저 상단 층 셀렉터에서 그 층을 선택(또는 [+ 층 추가])한 뒤** 버튼을
> 누른다. 버튼이 층 정보(`?floor=`)를 편집기에 넘기고, [저장 & 적용] 시 그 층 맵
> (`map_<층id>.png`)에만 반영된다 — 다른 층(예: 17F)은 건드리지 않는다.
> (직접 URL로 열 때 특정 층에 저장하려면 `http://<서버>:8910/?live=1&floor=<층id>`.)
...
> 이 worst-N 경로는 **단순 시각화에 그치지 않고, [저장 & 적용] 시 곧 운영의 EPFI
> 기준경로(routes)로 자동 반영**된다(§6 참조).
...
site 이름·**대상 층**을 확인 후 **[저장 & 적용]**. 산출물은 **선택된 층에 한해** 저장된다:

| 산출물 (층 = default/17F) | 산출물 (층 = 그 외, 예: floor2) | 내용 |
|--------|--------|------|
| `data/sites/<site>/map.png` | `map_<층id>.png` | 터치업된 깨끗한 평면도 = 그 층 2D맵 배경 |
| `floor.json` | `floor_<층id>.json` | 편집내역(삭제/개구부/Exit)·좌표계·**m_per_px 축척(자동)** |
| `evac_distfield.npz` | (공통) | **거리장 사전계산** — 운영 중 실시간 피난거리 조회용 |

"운영 서버 즉시 반영"이 켜져 있으면 :8900의 **해당 층 맵**이 곧바로 교체되고(`?floor=`로
그 층에만 POST), 맵 설정 화면이 **그 층으로 전환·자동 갱신**된다(새로고침 불필요).
다른 층은 영향받지 않는다. **축척 2점 수동 지정도 생략** — CAD가 mm 단위라 편집기가
정확한 m/px를 자동 기입한다.

### 6-1. 맵 교체와 함께 공간 요소도 자동 재세팅 (v1.9)

맵만 바뀌는 게 아니라, 맵 교체가 성공하면 편집기가 그 층의 **공간 요소를 CAD 도면
기준으로 함께 재세팅**한다(`PUT /api/site/floor-elements?floor=`). 새 맵은 옛 맵과 px
좌표계가 달라 옛 요소를 그대로 두면 어긋나기 때문이다:

- **피난경로(routes) 자동 반영** — §5의 **worst-N 최단경로가 운영 EPFI 기준경로로 자동
  등록**된다(id 접두 `auto-evac-`). **맵 설정에서 손으로 그린 경로는 보존**되고 자동경로만
  교체(`replace="auto"`)되므로, 수동 보정 경로는 사라지지 않는다.
- **출입구(Exit) → 통과선(exits) 반영** — §4에서 지정한 Exit가 운영의 출입구 통과선(붉은
  선)으로 반영된다. **'안쪽' 방향은 자동 추정**(Exit 중점 → 도면 bounds 중심 방향).
  부정확하면 **맵 설정에서 통과선의 안쪽 방향을 수동 보정**한다.
- **구역·병목은 자동으로 비워짐** — 옛 좌표를 새 맵에 남기지 않고 clear한다. IDR(구역)·
  CBS(병목)를 쓰려면 **새 맵 위에 맵 설정에서 다시 그려야 한다**.
