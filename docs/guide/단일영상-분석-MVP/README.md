...
> 🗺 **멀티카메라 2D맵 시스템**(카메라 N대 + 4대 정량지표, :8900)의 가이드는
> **[../멀티카메라-시스템/](../멀티카메라-시스템/)** 을 보세요. 이 문서는 기존
> **단일영상 분석 UI** 가이드입니다.
...
| **샘플 스트림 카드 클릭** | 그 RTSP로 바로 연결 (sample1 / in_out_counting / inout_sample2 / arxiepiskopi) |
...
| **지도 정합(N점)** | 지도 이미지 + CCTV↔지도 N점 + 축척(2점+m) | km/h (지도 위 표시) |

> **정렬 방향(선택)**: "정렬 방향 2점" 버튼으로 출구 방향 화살표(꼬리→머리)를 그리면
> **방향성 정렬도**(이동방향이 그 방향과 얼마나 일치하는지)를 측정·표출한다. 안 그리면 미표출.

### (b-3) 지도 정합 — 실제 지도 위에 표시
실제 평면도/지도 이미지를 올리고 **CCTV와 지도에 같은 지점을 N개(기본 10) 순서대로** 찍으면
(호모그래피) 사람을 **지도 좌표로 변환**해 표시한다. 단계: ① CCTV 점 → ② 지도 점 →
③ 축척(지도 2점+실거리 m) → ④ 정렬방향(선택) → ⑤ 통과선 in/out(선택). 점은 **바닥 지점**으로,
🗺 맵 토글 시 **업로드한 지도 위**에 위치·방향·정렬색이 표시된다. (상세 [webui-dev/14](../../webui-dev/14-map-registration.md))
**⑤ 통과선**을 그리면 대시보드에 **IN/OUT·재실** 카드가 함께 나온다(정렬도·속도·밀도와 동시).
...
- 대시보드: **현재 인원 / 세션 누적 / 밀도(+혼잡도) / 평균·최고 속도 / 가속도 / 평균·최대 체류 / 이동·정지 / (정렬 방향 그렸을 때) 정렬도 / 인원 추이 / 객체별 표.**
- 상단 우측 **`🗺 맵`** 토글로 top-down 맵 전환.

### (b) 2D 맵 뷰
![map](img/06-map.png)
흰 평면에 각 사람을 **발끝 지면 좌표 점 + 방향벡터**로 top-down 표시. (호모그래피/Depth면 미터 기준)
정렬 방향을 그렸으면 화살표가 **정렬색**(녹=정렬/황=가로질러/적=역류) + 군중 무게중심에서 **긴 "기준 방향" 벡터**.
...
### 방향성 정렬도 (정렬 방향 그렸을 때만)
각 사람의 이동방향과 그린 **기준(출구) 방향**의 **코사인**(−1~+1). +1 정렬·0 가로질러·−1 역류.
구역 평균 = 이동 객체들의 코사인 평균(정지 제외). **속도 크기는 무시, 방향만** 본다.
